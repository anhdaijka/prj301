    ...
    <div class="navbar-start">
        <div class="dropdown">
            ...
            <ul tabindex="-1" class="menu menu-sm dropdown-content bg-base-100 rounded-box z-1 mt-3 w-52 p-2 shadow">
                ...
                <li><a href="${pageContext.request.contextPath}/views/pages/jobs.jsp">Find job</a></li>
                <li><a href="${pageContext.request.contextPath}/views/pages/company.jsp">Company</a></li>
            </ul>
        </div>
                    ...
                    <c:choose>
                        <c:when test="${not empty sessionScope.user.role}">
                            ...
                            <span class="text-sm font-medium text-gray-700">
                                ${fn:toUpperCase(fn:substring(roleName, 0, 1))}${fn:toLowerCase(fn:substring(roleName, 1, fn:length(roleName)))}
                            </span>
                        </c:when>
                        <c:when test="${not empty sessionScope.userRole}">
                            <c:set var="roleName" value="${sessionScope.userRole}" />
                            <span class="text-sm font-medium text-gray-700">
                                ${fn:toUpperCase(fn:substring(roleName, 0, 1))}${fn:toLowerCase(fn:substring(roleName, 1, fn:length(roleName)))}
                            </span>
                        </c:when>
                    ...
                    <div class="dropdown dropdown-end">
                                ...
                                <div class="w-8 rounded-full">
                                    <c:choose>
                                        <c:when test="${not empty sessionScope.user.avatarurl}">
                                            <img src="${sessionScope.user.avatarurl}" alt="${sessionScope.user.name}" />
                                        </c:when>
                                        <c:otherwise>
                                            <img src="https://ui-avatars.com/api/?name=${fn:replace(sessionScope.user.name, ' ', '+')}&background=random&color=fff&size=128" 
                        ...
                        <ul tabindex="0" class="mt-3 p-2 shadow menu dropdown-content bg-base-100 rounded-box w-52">
                            <li class="menu-title">
                                ...
                                <span class="text-xs text-gray-500">${sessionScope.user.email}</span>
                            </li>
